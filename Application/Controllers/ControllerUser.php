    ...
    public function login()
    {
        ...
        // Si le post mail et password existe
            ...
            if(Helper::validMail($this->request->getParameter('mail-prime'))){
                ...
                // si le compte existe.
                $userLogin = new User();
                $user_id = $userLogin->login($mail, $password);

                if($user_id > 0){

                    $user = new User();
                    // Récupération d'un utilisateur
                    $loginUser = $user->getUser($user_id);

                    // On vérifie que l'utilisateur est actif
                    if($loginUser->isUserActive()){
                        // on crée 2 sessions user_id, et user_role afin de récupérer les informations si nécessaires
                        $this->request->getSession()->setAttribute('user_id', $loginUser->getUserId());
                        $this->request->getSession()->setAttribute('user_role', $loginUser->getRoleId());
                        ...
                        $log->setLogTypeId($loginUser->getUserId());
    ...
    public function confirm()
    {
        ...
        if($this->isLogged()){ // Redirection forcée si on est déjà connecté !
            ...
            if($userConfirm->getUserId() > 0){
                ...
                $user = new User();
                $currentUser = $user->getUser($userConfirm->getUserId());
